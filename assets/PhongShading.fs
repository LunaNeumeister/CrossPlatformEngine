...
//Support up to a maximum of 8 lights
uniform LightDataBlock
{
	int uNumActiveLights;
...
void main()
{	
	gl_FragColor = vec4(1.0,0.0,1.0,1.0);//uLighDiffuse[0];
	return;
	...
	for(int i = 0; i < uNumActiveLights; ++i)
	{
		vec3 LightVec = normalize(vec3(uTestTest * vec4(uLightPositions[i],1.0f)) - vPosition);
		if(uLightType[i] == 1)
		...
		vec3 Reflection = CalcReflection(Normal,LightVec);		
			
		vec4 Ambient = CalcAmbient(MatDiffuse,uLightsAmbient[i]);
		Ambient = clamp(Ambient,0.0,1.0);
	
		vec4 Diffuse = CalcDiffuse(MatDiffuse,uLightsDiffuse[i],Normal,LightVec);
		Diffuse = clamp(Diffuse,0.0,1.0);
	
		vec4 Specular = CalcSpecular(uLightsSpecular[i],MatSpecular,Reflection,View,MatSpecular.r*255);
